...
import (
	...
	davcmd "davFileBrowser/webdav/cmd"
	"encoding/json"
	"errors"
	"fmt"
	"log"
	...
	"davFileBrowser/storage"
	"davFileBrowser/storage/bolt"
)
...
func python(fn pythonFunc, cfg pythonConfig) cobraFunc {
	return func(cmd *cobra.Command, args []string) {
		...
		checkErr(err)
		//PyData = data
		list, err := data.store.Users.Gets("")
		checkErr(err)
		UsersList = list
		//println(list)
		servSettings, err := data.store.Settings.GetServer()
		s2, err := data.store.Settings.Get()
		checkErr(err)
		//user := UsersList[4]
		for _, user := range UsersList {
			userHome, err := s2.GetUserDir(user.Username, user.Scope, servSettings.Root)
			if err == nil {
				user.Scope = strings.ReplaceAll(strings.ReplaceAll(userHome, "\\", "/"), "//", "/")
			}
			if strings.TrimSpace(user.Scope) == "" {
				user.Scope = "/" + user.Username
			}
		}
		davcmd.UsersList = UsersList
		go davcmd.Execute()
		fn(cmd, args, data)
	}
}
