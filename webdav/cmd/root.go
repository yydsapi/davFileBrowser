...
import (
	...
	"davFileBrowser/webdav/configs"
	"fmt"
	"os"
	"strconv"
	"strings"

	_ "davFileBrowser/webdav/internal/common"
	"davFileBrowser/webdav/internal/server"
...
func createDefaultDirs() {
	...
	for _, dir := range dirs {
		...
		if _, err := os.Stat(dir); os.IsNotExist(err) {
			...
			if err = os.MkdirAll(dir, os.ModePerm); err != nil {
				log.Fatal().Err(err).Msg("Failed to create directories acl")
			}
...
func getHandlerConfigs(rootPath string, dav string, davs []*koanf.Koanf) (handlerConfigs []*server.HandlerConfig) {
	...
	for _, user := range UsersList {
		if strings.TrimSpace(user.Username) != "" && strings.TrimSpace(user.Password) != "" {
			handlerConfigs = append(handlerConfigs, &server.HandlerConfig{
				Prefix:   "/" + strings.ReplaceAll(user.Username, "/", "-"),
				PathDir:  user.Scope,
...
func Execute() {
	type Config struct {
		...
		MySqlConnStr    string `koanf:"mySqlConnStr"`
	...
	configs.MySqlConnStr = cfg.MySqlConnStr
	handlerConfigs := getHandlerConfigs(cfg.RootPath, cfg.Dav, result.K.Slices("davs"))
	/*
